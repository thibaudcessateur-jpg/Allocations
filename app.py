    ...
    with st.container(border=True):
        ...
        with cols[0]:
            ...
            if line.get("invalid_date"):
                st.markdown(
                    f"⚠ï¸ Date d'achat antérieure à la 1ère VL ({fmt_date(line.get('inception_date'))}).",
                )
        with cols[1]:
        ...
        with cols[4]:
            ...
            if st.button("🗑ï¸", key=f"del_{port_key}_{idx}", help="Supprimer"):
...
def render_portfolio_builder():
    ...
    if st.session_state.get("PP_RUN") and st.session_state.get("PP_PARAMS_HASH") != current_hash:
        ...
        st.info("Paramètres modifiés. Cliquez de nouveau sur '✅ Créer le portefeuille parfait'.")

    run_clicked = st.button("✅ Créer le portefeuille parfait", type="primary")
    if run_clicked:
        st.session_state["PP_RUN"] = True
        st.session_state["PP_PARAMS_HASH"] = current_hash

    if not st.session_state.get("PP_RUN"):
        st.info("Renseignez tous les paramètres puis cliquez sur '✅ Créer le portefeuille parfait'.")
        return
...
def render_app(run_page_config: bool = True):
    ...
    if st.session_state.get("DATE_WARNINGS"):
        with st.expander("⚠ï¸ Problèmes d'historique / dates de VL"):
