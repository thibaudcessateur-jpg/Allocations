...
# ------------------------------------------------------------
# Bloc "Frais & valeur créée" en mode analyse simple
# ------------------------------------------------------------
mode = st.session_state.get("MODE_ANALYSE", "compare")

def _years_between(d0: pd.Timestamp, d1: pd.Timestamp) -> float:
    # Années réelles (approx) — 365.25 pour tenir compte des années bissextiles
    return max(0.0, (d1 - d0).days / 365.25)

if mode in ("valority", "client"):
    ...
    if mode == "valority":
        ...
        title = "🧍 Portefeuille client — Frais & valeur créée"
    ...
    if brut > 0 and net >= 0 and val >= 0 and isinstance(start_min, pd.Timestamp):
        ...
        value_created = val - net                   # € gain vs capital réellement investi
        ...
        with st.container(border=True):
            st.markdown(f"### {title}")
            st.caption(
                f"Sur la période du **{fmt_date(start_min)}** au **{fmt_date(TODAY)}**, "
                f"avec des frais d’entrée de **{fee_pct:.2f}%**."
            )
            ...
            with c3:
                if value_per_year is not None:
                    st.metric("Valeur créée / an (moyenne)", to_eur(value_per_year))
                else:
                    st.metric("Valeur créée / an (moyenne)", "—")

            # Optionnel : TRI en rappel (plus pro que "moyenne/an")
            if irr is not None:
                st.markdown(f"- Rendement annualisé (XIRR) : **{irr:.2f}%**")
            else:
                st.markdown("- Rendement annualisé (XIRR) : **—**")

            # Petite phrase "vendeuse" mais factuelle
            if fees_paid > 0:
                ratio = (value_created / fees_paid) if fees_paid > 0 else None
                if ratio is not None:
                    st.markdown(
                        f"**Lecture rapide :** {to_eur(fees_paid)} de frais ont permis de générer "
                        f"**{to_eur(value_created)}** de valeur nette à date "
                        f"(**×{ratio:.1f}** la mise de frais)."
                    )
